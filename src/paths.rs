    ...
    for entry in fs::read_dir(source)? {
        ...
        if file_type.is_dir() {
            ...
            return Err(io::Error::new(
                ...
                "unsupported non-regular entry in legacy rvl config",
            ));
        }
    }
...
mod tests {
    ...
    fn witness_append_migrates_legacy_ledger() {
        let tmp = tempfile_dir("rvl-paths-witness");
        let home = tmp.join("home");
        let legacy = home.join(".epistemic").join("witness.jsonl");
        fs::create_dir_all(legacy.parent().unwrap()).unwrap();
        fs::write(&legacy, "{\"tool\":\"rvl\",\"outcome\":\"OLD\"}\n").unwrap();
    ...
    fn profile_read_migrates_legacy_profile_dir() {
        let tmp = tempfile_dir("rvl-paths-profile");
        let home = tmp.join("home");
