...
fn total_change_and_max_abs_delta_track_contributions() {
    ...
    acc.observe(cell_id(1, "a"), 10.0, 10.2, d1, c1, tie.next_value());

    let (d2, c2) = tol.apply(5.0, 7.0); // +2.0
    acc.observe(cell_id(2, "b"), 5.0, 7.0, d2, c2, tie.next_value());

    let (d3, c3) = tol.apply(3.0, 0.0); // -3.0
    acc.observe(cell_id(3, "c"), 3.0, 0.0, d3, c3, tie.next_value());
...
fn topk_ordering_is_deterministic() {
    ...
    acc.observe(cell_id(1, "b"), 1.0, 6.0, d1, c1, tie.next_value());
    let (d2, c2) = tol.apply(1.0, 4.0); // +3
    acc.observe(cell_id(2, "a"), 1.0, 4.0, d2, c2, tie.next_value());
    let (d3, c3) = tol.apply(1.0, 2.0); // +1
    acc.observe(cell_id(3, "c"), 1.0, 2.0, d3, c3, tie.next_value());
...
fn diffuse_when_topk_below_threshold() {
    ...
    acc.observe(cell_id(1, "a"), 0.0, 10.0, d1, c1, tie.next_value());
    let (d2, c2) = tol.apply(0.0, 5.0);
    acc.observe(cell_id(2, "b"), 0.0, 5.0, d2, c2, tie.next_value());
    let (d3, c3) = tol.apply(0.0, 3.0);
    acc.observe(cell_id(3, "c"), 0.0, 3.0, d3, c3, tie.next_value());
...
fn explainable_when_prefix_reaches_threshold() {
    ...
    acc.observe(cell_id(1, "a"), 0.0, 6.0, d1, c1, tie.next_value());
    let (d2, c2) = tol.apply(0.0, 3.0);
    acc.observe(cell_id(2, "b"), 0.0, 3.0, d2, c2, tie.next_value());
    let (d3, c3) = tol.apply(0.0, 1.0);
    acc.observe(cell_id(3, "c"), 0.0, 1.0, d3, c3, tie.next_value());
