...
get_latest_version() {
  payload=$(fetch "https://api.github.com/repos/cmdrvl/rvl/releases/latest" 2>/dev/null || true)
...
resolve_install_dir() {
  if [ -n "${RVL_INSTALL_DIR:-}" ]; then
    printf '%s' "$RVL_INSTALL_DIR"
    return 0
  fi
  ...
  if [ -z "${HOME:-}" ]; then
    fail "HOME is not set; set RVL_INSTALL_DIR instead"
  fi
...
make_temp_dir() {
  if command_exists mktemp; then
    tmp=$(mktemp -d 2>/dev/null || mktemp -d -t rvl-install)
  else
    tmp="${TMPDIR:-/tmp}/rvl-install-$$"
    mkdir -p "$tmp"
  fi
  printf '%s' "$tmp"
}
...
version="${RVL_VERSION:-}"
if [ -z "$version" ]; then
  info "No RVL_VERSION set; resolving latest release..."
  version=$(get_latest_version)
fi

version=$(normalize_version "$version")
target=$(detect_target)
asset="rvl-$version-$target.tar.gz"
base_url="https://github.com/cmdrvl/rvl/releases/download/$version"

install_dir=$(resolve_install_dir)
versioned_binary="$install_dir/rvl@$version"
active_binary="$install_dir/rvl"
...
info "Installing rvl $version for $target"
info "Install dir: $install_dir"

download "$base_url/$asset" "$archive_path"

skip_verify=0
if [ "${RVL_NO_VERIFY:-}" = "1" ]; then
  skip_verify=1
  warn "RVL_NO_VERIFY=1 set; skipping checksum verification"
fi
  ...
  if command_exists cosign; then
    if download "$base_url/SHA256SUMS.sig" "$sig_path" && download "$base_url/SHA256SUMS.pem" "$pem_path"; then
      if ! cosign verify-blob \
        ...
        --signature "$sig_path" \
        --certificate-identity "https://github.com/cmdrvl/rvl/.github/workflows/release.yml@refs/tags/$version" \
...
binary_path="$extract_dir/rvl"
if [ ! -f "$binary_path" ]; then
  binary_path=$(find "$extract_dir" -type f -name rvl | head -n 1 || true)
fi

if [ -z "$binary_path" ] || [ ! -f "$binary_path" ]; then
  fail "rvl binary not found in archive"
fi
...
if (cd "$install_dir" && ln -sf "rvl@$version" "rvl" 2>/dev/null); then
  :
else
...
info "Rollback: ln -sf \"rvl@$version\" \"$active_binary\""
